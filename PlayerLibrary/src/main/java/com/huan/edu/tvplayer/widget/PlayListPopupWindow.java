...
//        setAnimationStyle(R.style.EduTvPlayer_bottom_anim_style);
    ...
    public void showAtLocation(View parent, int gravity, int x, int y) {
        if (null != mRecyclerView) {
            mPlayPosition = PlayerSettings.getInstance(mContext).getPlayIndex();
            mRecyclerView.scrollToPositionWithOffsetStart(mPlayPosition);
            mRecyclerView.postDelayed(new Runnable() {
                @Override
                public void run() {
                    View view = mRecyclerView.getChildAt(mPlayPosition - mRecyclerView.getFirstVisiblePosition());
                    if (null != view) {
                        setItemActivated(view);
                        view.requestFocus();
                    }
                }
            }, 200);
